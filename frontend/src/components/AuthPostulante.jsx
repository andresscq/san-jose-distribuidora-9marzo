...
export const AuthPostulante = ({ alEntrar }) => {
  ...
  // Lógica para la barra de progreso (0 a 100%)
  const calcularProgreso = () => {
    const largo = datos.password.length;
    if (largo === 0) return 0;
    const porcentaje = (largo / 9) * 100;
  ...
  const handleSubmit = async (e) => {
    ...
    if (esRegistro && datos.password.length < 9)
      return setError("Mínimo 9 caracteres");
    if (esRegistro && datos.password !== datos.confirmarPassword)
      return setError("No coinciden");

    setCargando(true);
    try {
      const { confirmarPassword, ...enviar } = datos;
      const res = await api.post(
        esRegistro ? "/api/registro" : "/api/login",
        esRegistro ? enviar : { email: datos.email, password: datos.password },
      );
  ...
  return (
    <div className="max-w-md w-full relative">
      {/* Fondo decorativo con los colores de la marca */}
      ...
      <div className="relative bg-white p-10 md:p-12 rounded-[55px] shadow-[0_30px_80px_rgba(0,0,0,0.2)] border-t-[10px] border-yellow-400">
        ...
        {error && (
          <div className="bg-red-600 text-white p-4 rounded-2xl mb-6 text-[11px] font-black uppercase text-center italic shadow-lg animate-bounce">
            ⚠️ {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              name="email"
              type="email"
              placeholder="andres@distribuidora.com"
              className="w-full bg-slate-50 border-2 border-slate-200 rounded-[25px] px-7 py-4 outline-none font-bold text-slate-800 focus:border-green-600 transition-all shadow-inner"
              onChange={handleChange}
            />
          </div>
